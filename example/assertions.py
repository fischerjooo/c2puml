...
class PUMLValidator:
    ...
    def assert_specific_content(self, content: str, filename: str) -> None:
        ...
        if filename == "typedef_test.puml":
            ...
            assert 'TYPEDEF_MYBUFFER_MyBuffer ..> TYPEDEF_MYSTRING_MyString : <<uses>>' in content, "Missing MyBuffer uses MyString relationship"
            assert 'TYPEDEF_MYCALLBACK_MyCallback ..> TYPEDEF_MYBUFFER_MyBuffer : <<uses>>' in content, "Missing MyCallback uses MyBuffer relationship"
            assert 'TYPEDEF_MYCOMPLEX_MyComplex ..> TYPEDEF_MYLEN_MyLen : <<uses>>' in content, "Missing MyComplex uses MyLen relationship"
            assert 'TYPEDEF_MYCOMPLEX_MyComplex ..> TYPEDEF_MYSTRING_MyString : <<uses>>' in content, "Missing MyComplex uses MyString relationship"
            assert 'TYPEDEF_MYCOMPLEX_MyComplex ..> TYPEDEF_MYCALLBACK_MyCallback : <<uses>>' in content, "Missing MyComplex uses MyCallback relationship"
            assert 'TYPEDEF_MYCOMPLEX_MyComplex ..> TYPEDEF_LOG_LEVEL_T_log_level_t : <<uses>>' in content, "Missing MyComplex uses log_level_t relationship"
            assert 'TYPEDEF_MYCOMPLEXPTR_MyComplexPtr ..> TYPEDEF_MYCOMPLEX_MyComplex : <<uses>>' in content, "Missing MyComplexPtr uses MyComplex relationship"
            assert 'TYPEDEF_MYCOMPLEXARRAY_MyComplexArray ..> TYPEDEF_MYCOMPLEXPTR_MyComplexPtr : <<uses>>' in content, "Missing MyComplexArray uses MyComplexPtr relationship"
            ...
            assert 'TYPEDEF_NESTEDINFO_T_NestedInfo_t ..> TYPEDEF_LOG_LEVEL_T_log_level_t : <<uses>>' in content, "Missing NestedInfo_t uses log_level_t relationship"
            assert 'TYPEDEF_COMPLEXEXAMPLE_T_ComplexExample_t ..> TYPEDEF_NESTEDINFO_T_NestedInfo_t : <<uses>>' in content, "Missing ComplexExample_t uses NestedInfo_t relationship"
            assert 'TYPEDEF_COMPLEXEXAMPLE_T_ComplexExample_t ..> TYPEDEF_CE_STATUS_T_CE_Status_t : <<uses>>' in content, "Missing ComplexExample_t uses CE_Status_t relationship"
            ...
            assert 'TYPEDEF_REAL_T_real_t' in content, "Missing TYPEDEF_REAL_T_real_t class"
            assert 'TYPEDEF_MATH_OP_T_math_op_t' in content, "Missing TYPEDEF_MATH_OP_T_math_op_t class"
            assert 'TYPEDEF_ID_T_id_t' in content, "Missing TYPEDEF_ID_T_id_t class"
            assert 'TYPEDEF_STATUS_T_status_t' in content, "Missing TYPEDEF_STATUS_T_status_t class"
