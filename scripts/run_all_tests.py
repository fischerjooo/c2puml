...
def check_dependencies() -> bool:
    ...
    dependencies = {"pytest": "pytest --version", "coverage": "coverage --version"}
...
def run_pytest_tests(
    test_categories: List[str], verbosity: int = 1, with_coverage: bool = False
) -> bool:
...
Examples:
  ...
  python run_all_tests.py --stats            # Show test statistics
    ...
    parser.add_argument(
        "--stats", action="store_true", help="Show test suite statistics"
    )

    parser.add_argument(
        "--pattern", default="test_*.py", help="Test file pattern (unittest only)"
    )
    ...
    if args.pytest and deps_available:
        ...
        success = run_unittest_tests(
            test_pattern=args.pattern, verbosity=args.verbosity
        )
