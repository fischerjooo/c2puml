...
class PreprocessorEvaluator:
    ...
    def _expand_defined_operator(self, condition: str) -> str:
        ...
        condition = re.sub(r'defined\s*\(\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\)', replace_defined, condition)
        return condition
    ...
    def parse_preprocessor_blocks(self, tokens: List[Token]) -> List[PreprocessorBlock]:
        ...
        while i < len(tokens):
            ...
            if token.type == TokenType.PREPROCESSOR:
                ...
                if directive == PreprocessorDirective.IF or directive == PreprocessorDirective.IFDEF or directive == PreprocessorDirective.IFNDEF:
                    # Start new block
                    ...
                    block = PreprocessorBlock(
                        ...
                        is_active=self._should_activate_block(directive, condition, stack),
                        children=[],
                    ...
                    if stack:
                        ...
                        condition = self._extract_condition(token.value, directive)
                        current_block.is_active = self._should_activate_block(directive, condition, stack)
                        
                elif directive == PreprocessorDirective.ELSE:
                    if stack:
                        # Update current block
                        current_block = stack[-1]
                        current_block.is_active = self._should_activate_block(directive, "", stack)
    ...
    def _should_activate_block(self, directive: PreprocessorDirective, condition: str, stack: List[PreprocessorBlock]) -> bool:
    ...
    def _is_token_in_active_block(self, token_index: int, blocks: List[PreprocessorBlock]) -> bool:
...
class PreprocessorManager:
    ...
    def process_file(self, tokens: List[Token], defines: Optional[Dict[str, str]] = None) -> List[Token]:
        ...
        self.logger.debug(f"Preprocessor: {len(tokens)} tokens -> {len(filtered_tokens)} tokens")
        return filtered_tokens
        
    def add_defines_from_content(self, tokens: List[Token]):
        """Extract #define directives from tokens and add them to the evaluator."""
        for token in tokens:
            if token.type == TokenType.PREPROCESSOR and token.value.startswith('#define'):
