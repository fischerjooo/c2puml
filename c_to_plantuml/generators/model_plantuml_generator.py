    ...
    def _process_includes(self, file_model: FileModel) -> List[Dict]:
        ...
        for include in file_model.includes:
            ...
            if h_path and os.path.exists(h_path):
                # Use the optimized header parser
                from ..parsers.optimized_c_parser import OptimizedCParser
                prototypes, h_macros = OptimizedCParser.parse_header_file_optimized(h_path)
            else:
                prototypes, h_macros = [], []
...
def generate_plantuml_from_json(model_json_path: str, output_dir: str) -> None:
    ...
    generator = ModelPlantUMLGenerator()
