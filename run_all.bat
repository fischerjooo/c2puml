...
echo ----------------------------------------
REM Parse command line arguments for PlantUML JAR path
set "PLANTUML_JAR="
if not "%~1"=="" (
    set "PLANTUML_JAR=%~1"
    echo 📦 Using PlantUML JAR from argument: !PLANTUML_JAR!
    call picgen.bat "!PLANTUML_JAR!"
) else (
    REM Fallback: Try to find PlantUML JAR in VS Code extensions
