...
class TestParserTokenizerIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(
            ...
            f"Example directory not found: {self.example_dir}",
        )
    ...
    def test_parse_sample_c_file(self):
        ...
        for expected_func in expected_functions:
            self.assertIn(
                expected_func, func_names, f"Missing function: {expected_func}"
            )
        ...
        for expected_global in expected_globals:
            self.assertIn(
                expected_global, global_names, f"Missing global: {expected_global}"
            )
    ...
    def test_parse_typedef_test_files(self):
        ...
        if typedef_test_c_path.exists():
            file_model = self.parser.parse_file(
                typedef_test_c_path,
                typedef_test_c_path.name,
                str(typedef_test_c_path.parent),
            )
        ...
        if typedef_test_h_path.exists():
            file_model = self.parser.parse_file(
                typedef_test_h_path,
                typedef_test_h_path.name,
                str(typedef_test_h_path.parent),
            )
    ...
    def test_parse_complex_example_file(self):
        ...
        file_model = self.parser.parse_file(
            complex_example_path,
            complex_example_path.name,
            str(complex_example_path.parent),
        )
    ...
    def test_parse_math_utils_files(self):
        ...
        if math_utils_c_path.exists():
            ...
            expected_functions = [
                ...
                "subtract",
                "average",
            ]  # Updated based on actual content
    ...
    def test_end_to_end_parsing_workflow(self):
        ...
        self.assertGreaterEqual(
            len(file_model.functions), 6
        )  # sample.c has 6 functions
