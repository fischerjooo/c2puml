...
"""
    ...
    def test_cache_failed_include_search(self):
        ...
        result = self.parser._find_included_file(
            "missing_header.h", self.c_file, self.project_root
        )
    ...
    def test_existing_headers_not_cached(self):
        ...
        result = self.parser._find_included_file(
            "existing_header.h", self.c_file, self.project_root
        )
    ...
    def test_multiple_missing_headers_cached(self):
        ...
        result1 = self.parser._find_included_file(
            "missing_header1.h", self.c_file, self.project_root
        )
        self.assertIsNone(result1)

        # Search for second missing header
        result2 = self.parser._find_included_file(
            "missing_header2.h", self.c_file, self.project_root
        )
        self.assertIsNone(result2)
        ...
        self.assertTrue(
            any("missing_header1.h:" in include for include in cached_includes)
        )
        self.assertTrue(
            any("missing_header2.h:" in include for include in cached_includes)
        )
    ...
    def test_cache_key_uniqueness(self):
        ...
        try:
            ...
            result2 = self.parser._find_included_file(
                "missing_header.h", self.c_file, project_root2
            )
            self.assertIsNone(result2)
    ...
    def test_cache_with_different_include_formats(self):
        ...
        self.assertEqual(
            stats["cache_size"], 1
        )  # All three become the same key after stripping
    ...
    def test_cache_with_file_extensions(self):
        ...
        result2 = self.parser._find_included_file(
            "missing_header.hpp", self.c_file, self.project_root
        )
        self.assertIsNone(result2)

        # Test without extension
        result3 = self.parser._find_included_file(
            "missing_header", self.c_file, self.project_root
        )
        self.assertIsNone(result3)
    ...
    def test_cache_performance_improvement(self):
        ...
        self.assertLess(
            second_search_time, first_search_time * 0.1
        )  # At least 10x faster
    ...
    def test_cache_with_nested_directories(self):
        ...
        result = self.parser._find_included_file(
            "missing_header.h", nested_file, self.project_root
        )
        self.assertIsNone(result)
    ...
    def test_cache_with_special_characters(self):
        ...
        result1 = self.parser._find_included_file(
            "missing_header_file.h", self.c_file, self.project_root
        )
        self.assertIsNone(result1)

        # Test with dash
        result2 = self.parser._find_included_file(
            "missing-header-file.h", self.c_file, self.project_root
        )
        self.assertIsNone(result2)

        # Test with numbers
        result3 = self.parser._find_included_file(
            "missing_header_123.h", self.c_file, self.project_root
        )
        self.assertIsNone(result3)
