...
class TestGeneratorExactFormatCLI(UnifiedTestCase):
    ...
    def test_exact_requested_format(self):
        ...
        test_data = self.data_loader.load_test_data("111_gen_exact_requested")
        source_dir, config_path = self.data_loader.create_temp_files(test_data, "111_gen_exact_requested")
    ...
    def test_format_with_parameters(self):
        ...
        test_data = self.data_loader.load_test_data("111_gen_exact_params")
        source_dir, config_path = self.data_loader.create_temp_files(test_data, "111_gen_exact_params")
        temp_dir = os.path.dirname(source_dir)
        output_dir = os.path.join(os.path.dirname(temp_dir), "output")
        os.makedirs(output_dir, exist_ok=True)
