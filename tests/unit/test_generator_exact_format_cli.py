#!/usr/bin/env python3
"""
Test Generator Exact Formatting Through CLI Interface
"""
import os
import sys
import unittest
...
class TestGeneratorExactFormatCLI(UnifiedTestCase):
    """Test the exact PlantUML formatting through the CLI interface"""
    
    def test_exact_requested_format(self):
        """Test the exact format requested: public functions, empty line, private functions"""
        test_data = self.data_loader.load_test_data("generator_exact_format_cli_requested_format")
        source_dir, config_path = self.data_loader.create_temp_files(test_data, "generator_exact_format_cli_requested_format")
    ...
    def test_format_with_parameters(self):
        ...
        test_data = self.data_loader.load_test_data("generator_exact_format_cli_with_parameters")
        source_dir, config_path = self.data_loader.create_temp_files(test_data, "generator_exact_format_cli_with_parameters")
        temp_dir = os.path.dirname(source_dir)
        output_dir = os.path.join(os.path.dirname(temp_dir), "output")
        os.makedirs(output_dir, exist_ok=True)
