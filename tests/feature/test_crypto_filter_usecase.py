...
class TestCryptoFilterUseCase(BaseFeatureTest):
    ...
    def test_parser_with_crypto_filter(self):
        ...
        project_model = parser.parse_project(
            self.temp_dir, recursive_search=True, config=config
        )
            ...
            for filename in crypto_files:
                ...
                if i == 1:  # Pattern 2: crypto.*cfg.*
                    if "cfg" in filename.lower():
                        assert (
                            should_include
                        ), f"Pattern {i+1} should match {filename} (contains 'cfg')"
                    else:
                        assert (
                            not should_include
                        ), f"Pattern {i+1} should not match {filename} (no 'cfg')"
                    ...
                    assert (
                        not should_include
                    ), f"Pattern {i+1} should not match {filename} (case-sensitive)"
    ...
    def test_user_question_scenario(self):
        ...
        # User's target files
        target_files = ["Crypto_Cfg_Partitions.c", "Crypto_Cfg_JobQueues.c", "Crypto.c"]
