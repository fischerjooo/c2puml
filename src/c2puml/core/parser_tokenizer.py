...
def find_struct_fields(
    ...
    while pos < closing_brace_pos:
        ...
        # Start collecting tokens for this field
        field_start = pos
        field_tokens = []
        brace_count = 0
        ...
        while pos < closing_brace_pos and not found_semicolon:
            token = tokens[pos]
            ...
            if token.type == TokenType.LBRACE:
                ...
                brace_count -= 1
                # If we're back to brace_count == 0 and we're not at the main closing brace
                if brace_count == 0 and pos != closing_brace_pos:
                    # This is the end of a nested structure, continue looking for the field name
                    pass
            elif token.type == TokenType.SEMICOLON and brace_count == 0:
                # This is the semicolon that ends the field
                found_semicolon = True
            
            # Add non-whitespace tokens to field_tokens
            if token.type not in [TokenType.WHITESPACE, TokenType.COMMENT, TokenType.NEWLINE]:
                field_tokens.append(token)
            
            pos += 1
        
        # Parse the collected field tokens
        if field_tokens:
            # Check if this is a nested struct/union field
            if (len(field_tokens) >= 3 and 
                field_tokens[0].type in [TokenType.STRUCT, TokenType.UNION] and 
                field_tokens[1].type == TokenType.LBRACE):
                
                # Find the field name (the last identifier before the semicolon)
                field_name = None
                ...
                if field_name and field_name not in ["[", "]", ";", "}"]:
                    ...
                    if content:
                        ...
                        field_type = f"{field_tokens[0].value} {{ /*ANON:{encoded_content}:{field_name}*/ ... }}"
                    else:
                        field_type = f"{field_tokens[0].value} {{ ... }}"
                    
                    fields.append((field_name, field_type))
            
            # Check if this is a function pointer array field: type (*name[size])(params)
            elif (len(field_tokens) >= 8 and 
                  field_tokens[1].type == TokenType.LPAREN and 
                  field_tokens[2].type == TokenType.ASTERISK and 
            ...
            # Check if this is a function pointer field: type (*name)(params)
            elif (len(field_tokens) >= 5 and 
                  any(field_tokens[i].type == TokenType.LPAREN and field_tokens[i + 1].type == TokenType.ASTERISK 
                ...
                if func_ptr_start is not None:
                    ...
                    if name_end is not None:
                        ...
                        full_type = (field_type + " " + " ".join(t.value for t in func_ptr_start_tokens) + 
                                   " " + " ".join(t.value for t in func_ptr_end_tokens) + " " + param_type)
                        
                        if (field_name and field_name.strip() and full_type.strip() and 
                            field_name not in ["[", "]", ";", "}"]):
                            fields.append((field_name.strip(), full_type.strip()))
